...
const deletePhoto = async (req, res) => {
    ...
    try {
        ...
        if (!photo.userId.equals(reqUser._id)) {
            res.status(422).json({ errors: ["You do not have permission to delete this picture."] });
            return;
        }
        ...
        res.status(500).json({ errors: ["An error occurred while processing your request."] });
    }
};
